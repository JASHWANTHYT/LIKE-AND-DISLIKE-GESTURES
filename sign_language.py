...
lm_list = []  # Initialize lm_list array
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            # Drawing circles at finger tips
            for tip in finger_tips:
                x, y = int(hand_landmark.landmark[tip].x * w), int(hand_landmark.landmark[tip].y * h)
                cv2.circle(img, (x, y), 15, (255, 0, 0), cv2.FILLED)

            # Code for finger fold status check
            finger_fold_status = []
            for tip in finger_tips:
